...
{
	// #9: 初始化成员，避免 CreateFile/ReadFile 失败时析构对野指针 delete[] 崩溃
	FileRaw = NULL;
	FileSize = 0;
	FileStringsCount = 0;
	FileStringsMap = NULL;
	IniData.SectionCount = 0;
	IniData.Section = NULL;
...
bool INI_FILE::Parse()
{
	...
	for (DWORD CurrentStringNum = 0; CurrentStringNum < FileStringsCount; CurrentStringNum++)
	{
		...
		{
			// #10: 首节之前出现 key=value 时 CurrentSectionNum==-1，跳过避免越界写
			if (CurrentSectionNum != (DWORD)-1)
			{
				VariablesCount++;
