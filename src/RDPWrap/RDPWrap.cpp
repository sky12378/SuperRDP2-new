...
bool LoadConfig()
{
    ...
    IniFile = new (std::nothrow) INI_FILE(ConfigFile);
    ...
    if (!(IniFile->GetVariableInSection("Main", "LogFile", &LogFileVar)))
    {
        ...
        LogFile[255] = L'\0';
        const wchar_t* kDefaultName = L"rdpwrap.txt";
        const size_t kNameLen = wcslen(kDefaultName); // 11 字符，不含 终止符
        for (DWORD i = wcslen(LogFile); i > 0; i--)
        {
            if (LogFile[i] == '\\')
            {
                // i+1 之后必须能容纳 11 个字符 + 1 个终止符 = 12 个 wchar_t
                if (256 - (i + 1) >= (kNameLen + 1))
                {
                    memset(&LogFile[i + 1], 0x00, ((256 - (i + 1))) * sizeof(wchar_t));
                    memcpy(&LogFile[i + 1], kDefaultName, kNameLen * sizeof(wchar_t));
                    LogFile[i + 1 + kNameLen] = L'\0';
                }
                else
                {
                    // 路径过长，回退到简短默认值
                    wcsncpy(LogFile, kDefaultName, 255);
                    LogFile[255] = L'\0';
                }
                break;
            }
...
bool PatchSLPolicy(WORD VersionNumber)
{
    ...
    if ((VersionNumber == 0x0600) && Bool)
    {
        ...
        {
            _SLGetWindowsInformationDWORD = NULL;
        }
    ...
    if (VersionNumber == 0x0A00)
    {
        // Windows 10 / 11 (版本号编码为 0x0A00 = MAJOR(10)<<8 | MINOR(0))
        // uses SL Policy internal inline code
    }
